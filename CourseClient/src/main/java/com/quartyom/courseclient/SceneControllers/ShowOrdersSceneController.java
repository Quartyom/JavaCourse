...
public class ShowOrdersSceneController extends SceneController{
    Parent root;
    Label depositLabel;
    ...
    public ShowOrdersSceneController(QuApplication application) {
        super(application);
        try {
            root = FXMLLoader.load(application.getClass().getResource("show-platforms-view.fxml"));
            tileResource = application.getClass().getResource("order-platform-tile.fxml");
    ...
    public ArrayList<OrderForUser> getOrdersForUser(){
        try {
            QuPacket packet = new QuPacket("getOrdersForUser");
        ...
        try {
            ...
            if (response != null) {
                switch (response.type) {
                    case "unauthorized":
                        AuthSceneController.whereToReturn = new ShowOrdersSceneController(application);
    ...
    public void canselOrder(long orderId){
        try {
            QuPacket packet = new QuPacket("canselOrder");
            packet.data = orderId;
            packet.sessionId = application.sessionId;
            QuPacket response = ServerAdapter.doRequest(application.properties, packet);
            if (response != null) {
    ...
    public void update() {
        ...
        items.clear();
        for (OrderForUser item : getOrdersForUser()){
            try {
                ...
                Button canselButton = (Button) tile.lookup("#canselButton");

                String description = getDescriptionText(item);
                descriptionLabel.setText(description);

                String price = "Отменить (+%.2fр)".formatted(item.canselPriceKop / 100.0f);
                canselButton.setText(price);

                dateLabel.setText(item.orderDate.toString());

                canselButton.setOnAction(action -> {
                    canselOrder(item.id);
                    canselButton.setText("Отменено");
